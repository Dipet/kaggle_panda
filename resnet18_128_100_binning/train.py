...
class Model(LightningModule):
    ...
    def val_dataloader(self):
        dataset = TrainDatasetBinning(self.valid_df, self.data_dir, self.valid_transforms, 1, 128, 100)
...
trainer = Trainer(
    ...
    checkpoint_callback=ModelCheckpoint(filepath=f"checkpoints/{SAVE_NAME}" + "{epoch}_{kappa:.2f}", verbose=True, mode="max", monitor="kappa"),
)
trainer.fit(model)
